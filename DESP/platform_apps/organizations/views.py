...
class OrganizationViewPermission(BasePermission):
    ...
    def has_permission(self, request, view):
        if not request.user.is_authenticated:
            ...
            # supervisors only
            return request.user.user_type in ('S','A')
...
class OrganizationDetailViewPermission(BasePermission):
    ...
    def has_permission(self, request, view):
        if not request.user.is_authenticated:
            ...
            # supervisors or self
            return request.user.user_type in ('S','A') or (request.user.org.id == view.kwargs.get('id'))
        elif request.method in ('PUT', 'PATCH'):
            # supervisors - all, managers - self
            return request.user.user_type in ('S','A') or \
